...
Invoke-Command -Session $SESSIONS -ScriptBlock {
    try {
				...
				if ( -Not ( Test-Path $CLIENTPATH )) {
				    ...
				    throw "Client is broken."
				}
				
				# Check if SCCM Client Service is running
				$SERVICE = Get-Service -Name CcmExec -ErrorAction SilentlyContinue
				if ( $SERVICE -eq $NULL -or $SERVICE.Status -ne 'Running' ) {
				    Write-Host "SCCM Client service is not running."
				    throw "Client is broken."
				}
				
				# Check Site Assignment
				$SITECODE = ( Get-WmiObject -Namespace "root\ccm" -Class SMS_Client -ErrorAction SilentlyContinue ).AssignedSiteCode
				if ( [string]::IsNullOrEmpty( $SITECODE )) {
				    Write-Host "SCCM Client is not assigned to a site."
				    throw "Client is broken."
				}
...
if ( $CCMCLIENT -and $CCMCLIENT.ClientId ) {
    ...
    Write-Host "CCM_Client.ClientId property not found. SCCM Client may not be installed."
}   
...
if ($MP -and $MP.CurrentManagementPoint) {
    ...
    Write-Host "SMS_Authority.CurrentManagementPoint property not found. SCCM Client may not be installed."
}
        ...
        Write-Host "Client is broken. Running all repair actions..."
