...
{ 
    ...
    $CompCount = ($ComputerVar | measure-object).count
    $i = 0
    ...
    { 
        ...
        { 
            ...
            { 
                ...
                {          
                    ...
                    Write-Output -Verbose '===========================================' 
                    Write-host ("Running on $Computer. Computer number $i of $CompCount.")

                    $NewLine  
                 
                    Invoke-WmiMethod -ComputerName $Computer -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList '{00000000-0000-0000-0000-000000000021}' -ErrorVariable Error01
                    If ($Error01 -like '*Not found*') {
                    }
                    Else{
                        $NewLine 
                        ...
                        Write-Output -Verbose 'Waiting 5 minutes before running next SCCM client action...' 
                         ...
                         for ($a=300; $a -gt 1; $a--) {
                            Write-Progress -Activity "Working..." -SecondsRemaining $a -Status "Please wait"
                            Start-Sleep 1
                        } 
                ...
                {          
                    ...
                    Invoke-WmiMethod -ComputerName $Computer -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList '{00000000-0000-0000-0000-000000000113}' -ErrorVariable Error02
                    
                    If ($Error02 -like '*Not found*') {
                    }
                    Else{ 
                        $NewLine 
                ...
                {          
                    ...
                    Invoke-WmiMethod -ComputerName $Computer -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList '{00000000-0000-0000-0000-000000000108}' -ErrorVariable Error03
                    
                    If ($Error03 -like '*Not found*') {
                    }
                    Else{  
                        $NewLine 
                ...
                {          
                    ...
                    Write-Output -Verbose '==========================================' 
                    Write-Output -Verbose 'Windows Installer Source List Update Cycle' 
                    Write-Output -Verbose '==========================================' 
                    Write-host ("Running on $Computer. Computer number $i of $CompCount.")

                    $NewLine 
                 
                    Invoke-WmiMethod -ComputerName $Computer -Namespace root\ccm -Class sms_client -Name TriggerSchedule -ArgumentList '{00000000-0000-0000-0000-000000000032}' -ErrorVariable Error04
                    
                    If ($Error04 -like '*Not found*') {
                    }
                    Else{ 
                        $NewLine 
                        ...
                        # 5 min wait timer not needed because it is the last task on target computer
                     <# for ($a=300; $a -gt 1; $a--) {
                            Write-Progress -Activity "Working..." -SecondsRemaining $a -Status "Please wait"
                            Start-Sleep 1
                        } #>
        ...
        Write-host ("SCCM Action Bundle is complete on all intended computers") -foregroundcolor Green   
    } 
